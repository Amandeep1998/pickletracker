...
export default function Profile() {
  ...
  const [waSandboxKeyword, setWaSandboxKeyword] = useState(null);
  ...
  useEffect(() => {
    Promise.all([api.getProfile(), api.getWhatsAppStatus()])
      .then(([profRes, waRes]) => {
        ...
        if (wa.sandboxKeyword) setWaSandboxKeyword(wa.sandboxKeyword);
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-4 space-y-3">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-bold text-amber-900 mb-0.5">
                        {waSandboxKeyword ? 'Two quick steps to activate' : 'One step to activate'}
                      </p>
                      <ol className="text-xs text-amber-700 leading-relaxed list-none space-y-1 mt-1">
                        {waSandboxKeyword && (
                          <li>
                            <span className="font-bold">1.</span> First, send{' '}
                            <span className="font-bold font-mono bg-amber-100 px-1 rounded">join {waSandboxKeyword}</span>{' '}
                            to <span className="font-bold">{waBusinessNumber || 'our number'}</span> on WhatsApp to opt in to the sandbox.
                          </li>
                        )}
                        <li>
                          <span className="font-bold">{waSandboxKeyword ? '2.' : '1.'}</span> Then tap the button below — a message will be pre-typed, just hit <span className="font-bold">Send</span>.
                        </li>
                      </ol>
                    </div>
                  </div>
                  {waBusinessNumber ? (
                    <div className="space-y-2">
                      {waSandboxKeyword && (
                        <a
                          href={`https://wa.me/${waBusinessNumber.replace(/\D/g, '')}?text=join+${encodeURIComponent(waSandboxKeyword)}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center justify-center gap-2 w-full bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm py-2.5 rounded-xl transition-colors"
                        >
                          Step 1 — Join sandbox
                        </a>
                      )}
                      <a
                        href={`https://wa.me/${waBusinessNumber.replace(/\D/g, '')}?text=hi`}
                        target="_blank"
                        rel="noopener noreferrer"
                      ...
                      >
                        ...
                        {waSandboxKeyword ? 'Step 2 — ' : ''}Open WhatsApp &amp; Send "hi"
                      </a>
                    </div>
                  ) : (
                    <p className="text-xs text-amber-700 font-medium text-center">
                      Message <span className="font-bold">hi</span> to activate.
                    </p>
                  )}
                </div>
                  ...
                  <div className={`text-xs rounded-lg px-3 py-2.5 space-y-1 ${waTestResult.ok ? 'bg-[#f4f8e8] border border-[#91BE4D]/30 text-[#4a6e10]' : 'bg-red-50 border border-red-200 text-red-700'}`}>
                    {waTestResult.ok ? (
                      ...
                      <>
                        ...
                        {waTestResult.skipped && <p className="opacity-80">Env vars missing on server: TWILIO_ACCOUNT_SID or TWILIO_AUTH_TOKEN not set.</p>}
                        {waTestResult.envCheck && (
                          <p className="opacity-70 font-mono">
                            accountSid={waTestResult.envCheck.hasAccountSid ? '✓' : '✗'} &nbsp;
                            authToken={waTestResult.envCheck.hasAuthToken ? '✓' : '✗'} &nbsp;
                            from={waTestResult.envCheck.from || '✗'}
                          </p>
                        )}
                      </>
                    )}
